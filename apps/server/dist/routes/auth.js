...
exports.authRouter.post('/register', (req, res) => {
    ...
    const result = (0, users_1.registerUser)(username, password);
    if (!result.ok) {
    ...
    const loginResult = (0, users_1.loginUser)(username, password);
    if (!loginResult.ok) {
...
exports.authRouter.post('/login', (req, res) => {
    ...
    const result = (0, users_1.loginUser)(username, password);
    if (!result.ok) {
...
exports.authRouter.put('/password', auth_1.sessionAuth, (req, res) => {
    ...
    const user = (0, users_1.getUser)(req.sessionUsername);
    if (!user) {
    ...
    const loginResult = (0, users_1.loginUser)(req.sessionUsername, currentPassword);
    if (!loginResult.ok) {
        res.status(401).json({ error: 'Contraseña actual incorrecta' });
        return;
    }
    const result = (0, users_1.changePassword)(req.sessionUsername, user.recoveryCode, newPassword);
    if (!result.ok) {
...
exports.authRouter.put('/avatar', auth_1.sessionAuth, (req, res) => {
    const { avatar } = req.body;
    const result = (0, users_1.updateAvatar)(req.sessionUsername, avatar ?? null);
    if (!result.ok) {
...
exports.authRouter.post('/recover', (req, res) => {
    ...
    const result = (0, users_1.changePassword)(username, recoveryCode, newPassword);
    if (!result.ok) {
