...
export function IPTVBrowserModal({ open, onClose, listId, onSelect, roomId }: IPTVBrowserModalProps) {
    ...
    function handlePlayNow(entry: IPTVEntry) {
        onSelect(entry);
        onClose();
    }

    function handleAddToQueue(entry: IPTVEntry) {
        socket.emit('queue-add', {
            roomId,
            item: { type: 'movie', title: entry.name, streamUrl: entry.url, thumbnail: entry.logo ?? undefined },
        });
                    ...
                    <div className="flex-1 overflow-y-auto py-2">
                        ...
                        {!loading && filteredEntries.map((entry, i) => (
                            ...
                            >
                                ...
                                <span className="text-sm text-white/80 truncate flex-1">{entry.name}</span>
                                {isSearching && entry.group && (
                                    <span className="text-xs text-white/25 flex-shrink-0">{entry.group}</span>
                                )}
