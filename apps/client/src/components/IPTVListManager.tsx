...
import { RotateCcw, Pencil, Trash2, Plus, List } from 'lucide-react';
...
export function IPTVListManager() {
  ...
  function openCreate() {
    setModal({ open: true, target: null, name: '', url: '', saving: false });
  }

  function openEdit(list: IPTVList) {
    setModal({ open: true, target: list, name: list.name, url: list.url, saving: false });
  }
  ...
  async function handleSave() {
    ...
    try {
      if (target) {
        const { data } = await iptvApi.update(target.id, { name: name.trim(), url: url.trim() });
                  ...
                  <td className="py-3 pr-4 text-center">
                    <button
                      ...
                      className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${
                        list.enabled ? 'bg-violet-600' : 'bg-white/10'
                      }`}
                      title={list.enabled ? 'Desactivar lista' : 'Activar lista'}
                    >
                      <span
                        className={`inline-block h-3.5 w-3.5 transform rounded-full bg-white shadow transition-transform ${
                          list.enabled ? 'translate-x-[18px]' : 'translate-x-[3px]'
                        }`}
                      />
                    </button>
                  </td>
                  ...
                  <td className="py-3 text-right">
                    <div className="flex items-center justify-end gap-1">
                      <button
                        ...
                        title="Re-fetch y re-parse"
                        className="p-1.5 text-white/30 hover:text-violet-400 hover:bg-violet-500/10 rounded-lg transition-colors"
                      >
