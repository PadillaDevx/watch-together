...
export function RoomCard({ room, isAdmin, onDelete }: RoomCardProps) {
  ...
  return (
    <div className="group relative bg-surface-raised border border-white/[0.07] hover:border-violet-500/30 rounded-xl p-5 flex flex-col gap-4 transition-all duration-200 hover:bg-surface-card">
      {/* Delete button */}
      {isAdmin && onDelete && (
        <button
      ...
      <div>
        ...
        <div className="flex flex-wrap items-center gap-1.5 mt-2">
          ...
          {room.playerState.videoId && (
            <span className="text-xs px-2 py-0.5 rounded-full font-medium bg-violet-500/15 text-violet-400 flex items-center gap-1">
        ...
        <form onSubmit={handlePinSubmit} className="flex flex-col gap-2 pt-1 border-t border-white/[0.06]">
          ...
          <div className="flex gap-2">
            <input
              ...
              placeholder="000000"
              className={`flex-1 px-3 py-1.5 bg-white/5 border rounded-lg text-sm text-white placeholder-white/25 focus:outline-none focus:ring-1 font-mono tracking-widest ${pinError ? 'border-red-500/60 focus:ring-red-500/50' : 'border-white/10 focus:ring-violet-500/50'}`}
            />
            <button type="submit" className="px-3 py-1.5 bg-violet-600 hover:bg-violet-500 rounded-lg text-xs text-white transition-colors">
              OK
            </button>
